...
export function ProductEdit() {
  ...
  const handleAddInput = () => {
    setOptions([...options, { option_name: "", stock: "" }]);
  };
  ...
  return (
    <Card mt={10} mb={5} mx="auto" w={{ base: "90%", md: "80%", lg: "60%" }}>
      <CardHeader
      ...
      <CardBody p={5}>
        ...
        <SimpleGrid columns={{ base: 3, md: 4, lg: 5 }}>
          {product.productImgs.map((productImg, index) => (
            <Box
              position="relative"
              key={productImg.main_img_id}
              my="5px"
              onClick={() => handleRemoveMainImgSwitch(productImg.main_img_id)}
            >
              <Checkbox
                position="absolute"
                colorScheme="orange"
        ...
        <Box mt={3}>
          {options.map((option, index) => (
            <Flex key={index} mt={3} gap={3} alignItems="flex-end">
              <FormControl>
                <FormLabel fontWeight="bold">
                  {index !== 0 ? `${index}번째 상세옵션` : "기본 옵션"}
                </FormLabel>
                <Input
